...
namespace TheManager_GUI
{
    ...
    public partial class PlayerView : Window
    {
        ...
        public void InitializeCharts()
        {
            ...
            ChartView chartProgression = new ChartView(ChartType.LINE_CHART, FindResource("str_progression").ToString(), FindResource("str_level").ToString(), FindResource("str_years").ToString(), labels, false, 1, levels, -1, 250, 0, 100);
            chartProgression.RenderChart(panelProgression);
            ChartView chartGames = new ChartView(ChartType.LINE_CHART, FindResource("str_matchPlayed").ToString(), FindResource("str_games").ToString(), FindResource("str_years").ToString(), labels, false, 1, played, -1, 200, 0);
            chartGames.RenderChart(panelHistoryGames);
            ChartView chartGoals = new ChartView(ChartType.LINE_CHART, FindResource("str_goalsScored").ToString(), FindResource("str_goals").ToString(), FindResource("str_years").ToString(), labels, false, 1, goals, -1, 200, 0);
